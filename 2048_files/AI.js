...
function randMove(gameManager, moves, depth)
{
	...
	var playSafe = gameManager.playSafe;
	...
	var avg = (playSafe? 100000000000:0);
	for(var c in cells)
	{
		moves.push(cells[c].x*temp.size+cells[c].y);
		if(playSafe)
		{
			var tmp = bestMove(gameManager, moves depth-1);
			if(tmp < avg)
				avg = tmp;
		}
		else
...
function evaluate(g) 
{
	...
	for(var y = 0; y < 4; y++)
	{
		for(var x = 0; x < 4 && x >= 0; x += diff)
		{
			order.push({x: x, y: y});
		}
		diff = (diff == 1? -1:1);
	}
	var corner = g.grid.cells[0][0];
	var adder = 0;
	if(g.bestTile < 2)
		return g.score;
	if(corner != null)
	{
		...
		{
			if(g.grid.cells[order[i].x][order[i].y] != null)
			{
				if(g.grid.cells[order[i].x][order[i].y].value >= 2)
				{
					if(g.grid.cells[order[i-1].x][order[i-1].y] != null && g.grid.cells[order[i-1].x][order[i-1].y] >= g.grid.cells[order[i].x][order[i].y])
						adder += g.grid.cells[order[i].x][order[i].y].value*9;
					else
						break;
				}
			}
